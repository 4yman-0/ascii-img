...
fn get_characters_for_renderer(renderer_type: &RendererType) -> Vec<char> {
    match renderer_type {
        #[cfg(feature = "ascii-renderer")]
        RendererType::Ascii => Vec::from(ASCII_CHARS),
        #[cfg(feature = "ansi-renderer")]
        RendererType::Ansi => Vec::from(ANSI_CHARS),
        #[cfg(feature = "unicode-renderer")]
        RendererType::Unicode => Vec::from(UNICODE_CHARS),
    }
}
...
impl Renderer {
	...
	pub fn characters(mut self, characters: Option<Vec<char>>) -> Self {
		self.characters = characters
			.unwrap_or(get_characters_for_renderer(&self.renderer_type));
		self
	}
